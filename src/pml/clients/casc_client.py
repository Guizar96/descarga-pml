...
class CenaceCascClient:
    """Cliente para el servicio web SW-CASC (Cantidades Asignadas de Servicios Conexos, MDA únicamente)."""

    def __init__(self, settings: Settings):
        self.settings = settings
        self._session: Optional[aiohttp.ClientSession] = None

    async def __aenter__(self) -> "CenaceCascClient":
    ...
    def build_url(
        ...
        return (
            f"{self.settings.base_url}/SWCASC/SIM/{sistema}/MDA/{zc_segment}"
    ...
    async def fetch_json(
        ...
        if not self._session:
            raise RuntimeError("ClientSession no inicializada. Usa: async with CenaceCascClient(settings) as client")

        url = self.build_url(sistema, zonas, fecha_inicio, fecha_fin)
